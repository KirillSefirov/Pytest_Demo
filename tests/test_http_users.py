...
def test_post_register_new_user(base_url, first_name, last_name, email, password):
    register_user_response = users_api.register_new_user(base_url, first_name, last_name, email, password)
    ...
    user_profile = users_api.get_user_profile(base_url, email)
    assert user_profile.status_code == http.HTTPStatus.OK
    users_api.delete_existing_user(base_url, email, password)
...
def test_delete_registered_user(base_url, first_name, last_name, email, password):
    users_api.register_new_user(base_url, first_name, last_name, email, password)
    delete_user_response = users_api.delete_existing_user(base_url, email, password)
...
def test_get_user_profile(base_url, test_users):
    for user in test_users:
        response = users_api.get_user_profile(base_url, user['email'])
...
def test_patch_update_user_profile(base_url, test_users, new_first_name, new_last_name):
    for user in test_users:
        user_info = users_api.get_user_profile(base_url, user["email"])
        assert user_info.json()["firstName"] != new_first_name
        assert user_info.json()["lastName"] != new_last_name
        update_response = users_api.update_user_data(base_url,
                                                     new_first_name,
                                                     new_last_name,
                                                     user["email"],
                                                     user["password"])
        ...
        updated_user_info = users_api.get_user_profile(base_url, user["email"])
