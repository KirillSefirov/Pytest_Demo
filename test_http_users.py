...
def test_post_register_new_user(base_url, first_name, last_name, email, password):
    ...
    assert register_user_response.status_code == http.HTTPStatus.CREATED, f"response code should be 201, but was ${register_user_response.status_code}"
...
@pytest.mark.parametrize("new_first_name,new_last_name", helper.get_first_and_last_names_as_parameters())
def test_patch_update_user_profile(base_url, test_users, new_first_name, new_last_name):
    for user in test_users:
        ...
        update_response = api.update_user_data(base_url, new_first_name, new_last_name, user["email"], user["password"])
